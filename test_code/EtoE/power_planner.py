...
class MotorPowerPlanner():
    ...
    def __init__(self):
        ...
        self.BLUE_LOW_COLOR = np.array([100, 75, 75])
        self.BLUE_HIGH_COLOR = np.array([140, 255, 255])
        # 抽出する青色の塊のしきい値
    ...
    def avoid_color(self,frame,AREA_RATIO_THRESHOLD,LOW_COLOR,HIGH_COLOR):
        self.found_color = self.mpp.find_specific_color(frame,AREA_RATIO_THRESHOLD,LOW_COLOR,HIGH_COLOR)
        found_boolean: bool
        r_avoid: float
        l_avoid: float
        if self.found_color == None:
            r_avoid = 0
            l_avoid = 0
            found_boolean = False
