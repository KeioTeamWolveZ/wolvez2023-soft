...
class ARPowerPlanner():
    ...
    def __init__(self):
        self.arm_id = "1"
        # 各マーカーに対するxg,yg,zg
        self.marker_goal = {"2":[0.005,0.025,-0.023],"3":[-0.0045,0.004,0.041],"4":[0.005,0.041,-0.015],"5":[0.005,0.042,-0.015],\
            "6":[0,0.041,-0.015],"7":[0,0.043,-0.01],"11":[0.005,0.025,-0.0165],"16":[0.014,0.011,0.021],"68":[-0.005,0.01,-0.055]}
    ...
    def goalvec_maker(self,ar_info,goal_point,connecting_state,id):
        ...
        if connecting_state == 0:
            if self.arm_id in ar_info.keys():
                ...
                marker_1 = np.array([0.042479962,0.0090857051,0.14087037])
                # Low marker_1 = np.array([0.030354731,0.010103646,0.14465648])
        else:
            marker_1 = np.array([0.024985828,-0.015834280,0.13301809])
            # Low marker_1 = np.array([0.018225126,-0.0131257487,0.13601917])
