...
class ColorDetection():
    def __init__(self):
        ...
        """
        # 赤の色範囲を定義する
        self.r_low = np.array([0, 0, 128])  # 赤の最小値
        self.r_high = np.array([50, 50, 255])  # 赤の最大値

        # 緑の色範囲を定義する
        self.g_low = np.array([0, 128, 0])  # 緑の最小値
        self.g_high = np.array([50, 255, 50])  # 緑の最大値
        ...
        """
        # 赤色の範囲を定義する
        self.R_LOW_1 = np.array([0, 50, 50])
        self.R_HIGH_1 = np.array([10, 255, 255])
        self.R_LOW_2 = np.array([170, 50, 50])
        self.R_HIGH_2 = np.array([180, 255, 255])

        #green
        self.G_LOW = np.array([36, 25, 25])
        self.G_HIGH = np.array([70, 255, 255])
    ...
    def get_color_hsv(self,frame):
        ...
        # 色を抽出
        # 赤色
        r_hsv_mask1 = cv2.inRange(hsv,self.R_LOW_1,self.R_HIGH_1)
        r_hsv_mask2 = cv2.inRange(hsv,self.R_LOW_2,self.R_HIGH_2)
        # ２つの領域を統合
        r_hsv_mask = r_hsv_mask1 | r_hsv_mask2
        # 緑色
        g_hsv_mask = cv2.inRange(hsv,self.G_LOW,self.G_HIGH)
        # 青色
        b_hsv_mask = cv2.inRange(hsv,self.B_LOW,self.B_HIGH)
